...
export default function Hero({ onStart, onVoiceResult, draws4D, drawsToto, lang = 'en' }) {
  return (
    // Outer: full-height column — content grows in the middle, scroll hint stays at bottom
    <div className="relative min-h-screen flex flex-col items-center px-6 pt-20">
      <Lantern side="left" />
      <Lantern side="right" />
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center w-full py-8">
        ...
        <div style={{ animation: 'slideUp 0.6s ease-out both' }}>
          <h1 className="font-black leading-none mb-2"
            style={{ fontSize: 'clamp(4rem, 12vw, 8rem)', color: '#faf5f0' }}>
        ...
        <p className="text-lg md:text-xl max-w-md mb-2"
        ...
        <button
          onClick={onStart}
          className="btn-red relative px-12 py-5 rounded-full font-black text-xl uppercase tracking-widest text-white active:scale-95"
        ...
        {/* Stats row — draw dates + jackpot */}
        <div
          className="grid grid-cols-1 sm:flex sm:flex-wrap justify-center gap-2 mt-8 w-full max-w-sm sm:max-w-none"
      ...
      {/* Scroll hint — always below the content, never overlapping */}
      <div className="flex flex-col items-center gap-2 pb-6" style={{ opacity: 0.2 }}>
